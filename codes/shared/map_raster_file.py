...
def map_raster_file(sFilename_in,
    ...
    if aFilename_vector_in is not None:
        for sFilename, sColor in zip(aFilename_vector_in, aColor_vector_in):
            aPolygon = []
            aPolyline = []
            aColor = []
            ...
            if len(aPolygon) > 0:
                ...
                pPC = PatchCollection(aPatch, edgecolor=aColor,
                                          facecolor='none', linewidths=1.0,
                                          transform=pProjection_data)
                ax.add_collection(pPC)

            if len(aPolyline) > 0:
                pLC = LineCollection(aPolyline,  edgecolor=aColor, linestyle='--',
    ...
    if aLabel_legend_in is not None:
        ...
        for i in range(1, nlegend, 1):
            ...
            dLocation = dLocation - i * 0.05 - 0.03
            ax.text(0.03, dLocation, sText,
                    verticalalignment='top', horizontalalignment='left',
                    transform=ax.transAxes,
                    color='black', fontsize=10)
