...
def scatter_plot_multiple_data(aData_x,
    ...
    rect_scatter_mini = [dX_mini, dY_mini, width_mini, width_mini]
    ...
    for iax in range(len(ax_scatter_all)):
        ...
        if iax == 0:
            ...
            ax_scatter.set_xmargin(0.15)
            ax_scatter.set_ymargin(0.15)

            ax_scatter.set_xlabel(sLabel_X, fontsize=16, labelpad=0)
            ax_scatter.set_ylabel(sLabel_Y, fontsize=16, labelpad=-4)
            ...
            ax_scatter.set_xlim(dMin_x-0.1, dMax_x )
            ax_scatter.set_ylim(dMin_y-0.1, dMax_y )
        else:
            ax_scatter.set_xlim(0.5, 0.95)
            ax_scatter.set_ylim(0.5, 0.95)
            pass
        ...
        line = mpl.lines.Line2D([-1.1, 1], [0, 0], color='black', linestyle='dashed')
        #transform = ax_scatter.transAxes
        #line.set_transform(transform)
        ax_scatter.add_line(line)

        line = mpl.lines.Line2D([0, 0], [-1.1, 1], color='black', linestyle='dashed')
        #transform = ax_scatter.transAxes
        #line.set_transform(transform)
        ax_scatter.add_line(line)
        ...
        if iax == 0:
            if iFlag_flipped == 1:
                ...
                aLegend_artist = aLegend_artist[::-1]
                aLabel = aLabel[::-1]
        ...
        if iax == 0:
            ...
            if iFlag_miniplot == 1:
                ...
                line = mpl.lines.Line2D(
                    [0.75, 0.95], [0.95, 0.95], color='black', linestyle='dotted')

                line.set_transform(transform)
                ax_scatter.add_line(line)

                line = mpl.lines.Line2D(
                    [0.75, 0.95], [0.75, 0.75], color='black', linestyle='dotted')
                ...
                # vertical
                line = mpl.lines.Line2D(
                    [0.75, 0.75], [0.75, 0.95], color='black', linestyle='dotted')

                line.set_transform(transform)
                ax_scatter.add_line(line)

                line = mpl.lines.Line2D(
                    [0.95, 0.95], [0.75, 0.95], color='black', linestyle='dotted')

                line.set_transform(transform)
                ax_scatter.add_line(line)

                line = mpl.lines.Line2D([(dX_mini-left)/width, 0.75], [
                                     dY_mini + width_mini, 0.75], color='black', linestyle='dotted')
                transform = ax_scatter.transAxes
                line.set_transform(transform)
                ax_scatter.add_line(line)

                line = mpl.lines.Line2D([(dX_mini+width_mini-left)/width,  0.95], [dY_mini + width_mini,
                                     0.75], color='black', linestyle='dotted')
                transform = ax_scatter.transAxes
                line.set_transform(transform)
                ax_scatter.add_line(line)
