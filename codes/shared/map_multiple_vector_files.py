...
from pyearth.toolbox.date.timer import pytimer
...
from pyearth.visual.map.map_servers import StadiaStamen, EsriTerrain, EsriHydro, EsriRelief
...
def map_multiple_vector_files(aFiletype_in,
    ...
    if aExtent_in is not None:
        minx, maxx, miny, maxy = aExtent_in
        if iFlag_filter == 1:
    ...
    for i in range(nFile):
        ...
        for pFeature in pLayer:
            ...
            if sGeometry_type =='POINT':
                ...
                if dLongitude_highlight_in is not None and dLatitude_highlight_in is not None:
                    #calculate distance
                    dLongitude_from = float(aCoords_gcs[:,0])
                    dLatitude_from = float(aCoords_gcs[:,1])
                    dDistance = calculate_distance_based_on_longitude_latitude(dLongitude_from, dLatitude_from, dLongitude_highlight_in, dLatitude_highlight_in)
                    if dDistance < 5000:
                        #highlight the polygon
                        sColor = 'darkorange'
                        #also change the symbol and its size
                        aColor.append(sColor)
                        aPoint_x.append(dLongitude_from)
                        aPoint_y.append(dLatitude_from)
                        aSize.append(200)
                        #use a different marker such as star
                        #aMarker.append('*')
                        aMarker.append('^')
                    else:
                        aColor.append(sColor)
                ...
                if sGeometry_type =='LINESTRING':
                    ...
                    aThickness.append(iThickness* 1.2)
                    aColor.append(sColor)
        ...
        if len(aPoint_x) > 0:
            #scatter = ax.scatter(aPoint_x, aPoint_y, c=aColor,
            #                     s=aSize, marker=aMarker,
