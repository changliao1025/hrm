...
def create_nested_latlon_mesh(dLongitude_left_in,
                       dLatitude_bot_in,
                       dResolution_degree_in,
                       ncolumn_in, nrow_in,

                       sFilename_output_in,
                    iColumn_index_highlight_in=None,
                          iRow_index_highlight_in=None,
                          nNested = None,
                       pBoundary_in=None):
    """
    _summary_
    ...
    def add_cell_into_list(aList, lCellID, iRow, iColumn, dLongitude_center, dLatitude_center, aCoords ):
        ...
        pLatlon = convert_gcs_coordinates_to_cell(3, dLongitude_center, dLatitude_center, aCoords)
    ...
    for iRow in range(1, nrow_in+1):
        for iColumn in range(1, ncolumn_in+1):
            ...
            if ( iFlag == True ):
                if (iColumn == iColumn_index_highlight_in+1) and (iRow == iRow_index_highlight_in+1):
                    #highlight the cell
                    #divide this cell into 1
                    xspacing1 = xspacing/nNested
                    yspacing1 = yspacing/nNested
                    for iRow_dummy in range(1, nNested+1):
                        for iColumn_dummy in range(1, nNested+1):
                            lCellID = (iRow_dummy-1) * ncolumn_in + iColumn_dummy
                            x11 = x1 + ((iColumn_dummy-1) * xspacing1)
                            y11 = y1 + ((iRow_dummy-1) * yspacing1)
                            x21 = x1 + ((iColumn_dummy ) * xspacing1)
                            y21 = y1 + ((iRow_dummy-1) * yspacing1)
                            x31 = x1 + ((iColumn_dummy ) * xspacing1)
                            y31 = y1 + ((iRow_dummy ) * yspacing1)
                            x41 = x1 + ((iColumn_dummy-1) * xspacing1)
                            y41 = y1 + ((iRow_dummy ) * yspacing1)
                            coordinates = [(x11, y11), (x21, y21), (x31, y31), (x41, y41), (x11, y11)]
                            ring = ogr.Geometry(ogr.wkbLinearRing)
                            for x, y in coordinates:
                                ring.AddPoint(x, y)
                            ...
                            dLongitude_center = np.mean(aCoords_gcs[0:4,0])
                            dLatitude_center = np.mean(aCoords_gcs[0:4,1])
                            aLatlon, dArea = add_cell_into_list(aLatlon, lCellID, iRow_dummy, iColumn_dummy, dLongitude_center,dLatitude_center, aCoords_gcs )                            #save feature
